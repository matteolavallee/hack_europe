...
async def get_audio_contents(care_receiver_id: Optional[str] = Query(None)):
...
async def create_audio_content(payload: CreateAudioContentPayload):
    contents = json_store_service.get_audio_contents()
...
async def send_audio_now(item_id: str):
    contents = json_store_service.get_audio_contents()
...
async def schedule_audio(item_id: str, payload: ScheduleAudioPayload):
    contents = json_store_service.get_audio_contents()
    item = next((c for c in contents if c.get("id") == item_id), None)
    if not item:
        raise HTTPException(status_code=404, detail="Audio content not found")
...
async def toggle_recommendable(item_id: str, payload: ToggleRecommendablePayload):
    contents = json_store_service.get_audio_contents()
...
async def delete_audio_content(item_id: str):
    contents = json_store_service.get_audio_contents()
...
async def trigger_suggestion(payload: DemoSuggestionPayload):
    actions = json_store_service.get_device_actions()
...
async def get_next_actions(care_receiver_id: Optional[str] = Query(None)):
    actions = json_store_service.get_device_actions()
    return [DeviceAction(**a) for a in actions]

@router.post("/device/response")
async def submit_device_response(payload: DeviceResponsePayload):
    actions = json_store_service.get_device_actions()
    # Simple strategy: just clear the action from the queue once responded to
...
async def submit_help_request(payload: HelpRequestPayload):
    events = json_store_service.get_events()
    events.append({
        "id": f"ev-{uuid.uuid4().hex[:8]}",
        "care_receiver_id": "default", # Should fetch proper ID
